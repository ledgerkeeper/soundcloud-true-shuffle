(() => {
        ...
        try {
            ...
            if (!triedDetails && (playlistId || playlistUri)) {
                try {
                    ...
                    if (details)
                        return fetchPlaylistTracks(details, token, clientId, true);
                }
                catch (detailErr) {
    ...
    async function toPlayableSoundCloudUrl(url) {
        ...
        if (playableUrlCache.has(url))
            return playableUrlCache.get(url);
        let parsed;
        try {
            parsed = new URL(url);
        }
    ...
    async function preResolvePlayableQueueUrls(urls) {
        ...
        if (!uniqueUrls.length)
            return [];
        const resolvedUnique = await mapWithConcurrency(uniqueUrls, PLAYABLE_URL_RESOLVE_CONCURRENCY, async (url) => (await toPlayableSoundCloudUrl(url)) || url);
    ...
    async function playNextInQueue(tabId, options = {}) {
        const navigationMode = options?.navigationMode === "hard" ? "hard" : "content";
        const targetTabId = tabId || activeTabId;
        if (!targetTabId || !playbackQueueUrls.length || currentIndex >= playbackQueueUrls.length)
            return;
        ...
        if (playableUrl !== url) {
            ...
            if (playbackQueue[currentIndex])
                playbackQueue[currentIndex].url = playableUrl;
            await persistPlaybackState();
        }
        ...
        try {
            ...
            if (request.mode === "likes") {
                ...
                const repostsPromise = tryFetchAll([
                    ...
                    `https://api-v2.soundcloud.com/users/${userId}/reposts`,
                    `https://api-v2.soundcloud.com/stream/users/${userId}/reposts`,
                ], token, clientId).then((r) => r.items).catch(() => []);
