...
const LOG_PREFIX = "[SC Shuffle Listener]";
...
function waitForExpectedPageReady(targetUrl, timeoutMs = 1800) {
  ...
  return new Promise((resolve) => {
    const tick = () => {
      ...
      if (pageMatches && (pageControl || footerPlayLabel.includes("play current"))) {
        resolve(true);
        return;
      }
      if (Date.now() - startedAt > timeoutMs) {
...
async function navigateAndPlay(url) {
  ...
  if (footerBtn && footerPlayLabel.includes("play current")) {
    state.lastAdvanceKickAt = Date.now();
    const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PLAY_CURRENT");
    if (!nativeClicked) footerBtn.click();
    log("NAVIGATE_AND_PLAY: clicked footer 'Play current' for expected track", { url });
  } else {
    const immediatePageBtn = getPagePlayButton(url);
    if (immediatePageBtn) {
      state.lastAdvanceKickAt = Date.now();
      const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PAGE_PLAY", url);
      if (!nativeClicked) immediatePageBtn.click();
      log("NAVIGATE_AND_PLAY: clicked page play for expected track", { url });
    }
  }
...
function ensurePlayingOnce(expectedUrl) {
  const now = Date.now();
  if (!ensurePlayingOnce._lastKickAt) ensurePlayingOnce._lastKickAt = 0;
  ...
  const canKick = now - ensurePlayingOnce._lastKickAt > 1200;
  const footerPlayLabel = (
    footerBtn?.getAttribute("aria-label") ||
    footerBtn?.getAttribute("title") ||
    ""
  ...
  if (expectedUrl && pageMatches && !playerOnExpected && canKick) {
    if (footerBtn && footerPlayLabel.includes("play current")) {
      log("ENSURE_PLAYING: load expected track via footer 'Play current'", {
        expectedUrl,
        footerTrackHref,
      });
      ensurePlayingOnce._lastKickAt = now;
      const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PLAY_CURRENT");
      if (!nativeClicked) footerBtn.click();
      return false;
    }
    ...
    if (pageBtn) {
      log("ENSURE_PLAYING: load expected track via page play", { expectedUrl, footerTrackHref });
      ensurePlayingOnce._lastKickAt = now;
      const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PAGE_PLAY", expectedUrl);
      if (!nativeClicked) pageBtn.click();
      return false;
    }
  }
...
function suppressUnexpectedPlaybackWhileAdvancing() {
  ...
  if (
    ...
    if (footerBtn && footerPlayLabel.includes("play current")) {
      state.lastAdvanceKickAt = now;
      const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PLAY_CURRENT");
      if (!nativeClicked) footerBtn.click();
      log("Forced footer 'Play current' while waiting for shuffle advance", {
        expectedUrl: state.pendingExpectedUrl,
        footerTrackHref,
      });
    } else {
      const pageBtn = getPagePlayButton(state.pendingExpectedUrl);
      if (!pageBtn) return;
      state.lastAdvanceKickAt = now;
      const nativeClicked = dispatchNativeClick("SC_SHUFFLE_PAGE_PLAY", state.pendingExpectedUrl);
      if (!nativeClicked) pageBtn.click();
      log("Forced page play while waiting for shuffle advance", {
        expectedUrl: state.pendingExpectedUrl,
        footerTrackHref,
      });
    }
...
function bindTimelineEndSeekWatcher() {
  if (bindTimelineEndSeekWatcher._bound) return;
  bindTimelineEndSeekWatcher._bound = true;
