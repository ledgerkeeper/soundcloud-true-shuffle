...
(() => {
    ...
    function ensureGlassStyles() {
        if (document.getElementById("sc-shuffle-glass-style"))
            return;
  ...
  `;
    ...
    function getModernTrackPageButton(expectedTitle = null) {
        const normalizedExpectedTitle = normalizeTrackText(expectedTitle);
        const buttonSelector = 'button[aria-label="Play"], button[aria-label="Pause"], button[aria-label^="Play "], button[aria-label^="Pause "]';
        const headings = Array.from(document.querySelectorAll('main section[aria-label="Track header"] h1[title], main section[aria-label="Track header"] h2[title], h1[title], h2[title], h1, h2'));
        const candidates = [];
        for (const heading of headings) {
            const headingTitle = normalizeTrackText(heading.getAttribute("title") || heading.textContent);
            if (normalizedExpectedTitle && headingTitle !== normalizedExpectedTitle)
                continue;
            ...
            for (let depth = 0; container && depth < 6; depth += 1, container = container.parentElement) {
                ...
                if (button && isValidPagePlayButton(button) && !candidates.includes(button))
        ...
        if (!normalizedExpectedTitle) {
            const directButtons = Array.from(document.querySelectorAll(buttonSelector)).filter((button) => isValidPagePlayButton(button));
    ...
    function getPageTransportButton(expectedUrl = null, expectedTitle = null) {
    ...
    function waitForExpectedPageReady(targetUrl, expectedTitle = null, timeoutMs = 5000) {
    ...
    function isValidPagePlayButton(btn) {
        if (!btn)
            return false;
        if (btn.closest(".playControls"))
            return false;
        if (btn.closest(".relatedSounds, .trackList, .soundList, [class*='queue'], [class*='related'], aside"))
            return false;
    ...
    function isPlayStartButton(btn) {
        ...
        if (label.includes("pause"))
            return false;
        if (className.includes("sc-button-pause"))
            return false;
        if (label.includes("play"))
            return true;
        if (className.includes("sc-button-play"))
            return true;
        return false;
    }
    ...
    function requestExpectedPagePlay(expectedUrl, source, expectedTitle = null) {
    ...
    async function navigateAndPlay(url, expectedTitle = null) {
        ...
        if (!pageReady) {
            ...
            if (urlsRoughlyMatch(location.href, url)) {
                ...
                ensurePlayingWithRetry(url, { attempts: 14, intervalMs: 450 })
                    .then((isPlaying) => {
                    if (!isPlaying)
                        log("NAVIGATE_AND_PLAY: late DOM play was not confirmed", { url });
                })
        ...
        const footerPlayLabel = (footerBtn?.getAttribute("aria-label") ||
            ...
            "").toLowerCase();
        const playerOnExpected = !!footerTrackHrefAfterNavigation &&
    ...
    function ensurePlayingOnce(expectedUrl, expectedTitle = null) {
        const now = Date.now();
        ...
        const pagePlaying = isPlaying(pageTransport);
        const playerOnExpected = !!expectedUrl &&
            !!footerTrackHref &&
        ...
        if (expectedUrl) {
            if (pageMatches && pagePlaying === true)
                return true;
            if (playerOnExpected) {
                const footerPlaying = isPlaying(footerBtn) === true;
                if ((audio && audio.paused === false) || footerPlaying)
                    return true;
            }
        }
        else {
        ...
        if ((playerOnExpected || !expectedUrl) && (audioPaused === true || audioPaused === null) && canKick) {
            ...
            const shouldClick = (audioPaused === true) ||
    ...
    function getPagePlayButton(expectedUrl = null) {
        ...
        if (expectedUrl) {
            ...
            for (const link of matchingLinks) {
                const container = link.closest(".listenEngagement, .fullHero, .soundActions, .trackItem, li, article, section, div") ||
                    link.parentElement;
                if (!container)
                    continue;
                const playBtn = container.querySelector("a.playButton, a[class*='playButton'], a[class*='sc-button-play'], a[role='button'][title^='Play'], a[role='button'][aria-label^='Play'], button.playButton, button[class*='playButton'], button[class*='sc-button-play'], button[aria-label^='Play'], button[title^='Play']");
                if (isValidPagePlayButton(playBtn) && isPlayStartButton(playBtn))
                    return playBtn;
            }
            ...
            for (const sel of primarySelectors) {
                ...
                if (!isValidPagePlayButton(btn) || !isPlayStartButton(btn))
                    continue;
                return btn;
            }
            return null;
    ...
    function reportTrackFinished(reason, extra = null) {
        ...
        const shouldHoldShuffleAdvance = repeatState.mode === "one" ||
            repeatState.mode === "on";
    ...
    function suppressUnexpectedPlaybackWhileAdvancing() {
        ...
        const footerTrackHref = getCurrentTrackHref();
        const expectedModernPagePlaying = !!state.pendingExpectedUrl &&
        ...
        if (state.pendingExpectedUrl &&
            urlsRoughlyMatch(location.href, state.pendingExpectedUrl) &&
            now - state.lastAdvanceKickAt > 450) {
            requestExpectedPagePlay(state.pendingExpectedUrl, "ADVANCE_SUPPRESSION", state.pendingExpectedTitle);
        }
        if (now - state.lastAdvanceSuppressionAt < 250)
            return;
        const footerBtn = getPlayButton();
        const playingByAria = isPlaying(footerBtn);
    ...
    function parseTimeToSeconds(text) {
        ...
        if (!parts.length || parts.some((p) => p === "" || Number.isNaN(Number(p))))
            return null;
    ...
    function getCurrentTrackHref() {
        const link = document.querySelector("a.playbackSoundBadge__titleLink") ||
            document.querySelector('a[href*="soundcloud.com"][class*="playbackSoundBadge__titleLink"]');
    ...
    function getPlayButton() {
        return (document.querySelector('button[aria-label][class*="playControl"]') ||
    ...
    function getNativeRepeatState() {
        ...
        if (!btn)
            return { enabled: false, mode: "off", reason: "missing-button" };
        const label = `${btn.getAttribute("aria-label") || ""} ${btn.getAttribute("title") || ""}`.toLowerCase();
        const className = String(btn.className || "").toLowerCase();
        const pressedAttr = btn.getAttribute("aria-pressed");
        const isPressed = pressedAttr === "true";
        const isExplicitOffByText = label.includes("enable repeat") ||
            label.includes("turn on repeat") ||
            label.includes("repeat off");
        const isExplicitOnByText = label.includes("disable repeat") ||
        ...
        const activeByClass = className.includes("sc-button-selected") ||
        ...
        const isRepeatAllByClass = className.includes("repeatcontrol--all") ||
            className.includes("m-all");
        const isRepeatOneByClass = className.includes("repeatcontrol--one") ||
            className.includes("m-one");
    ...
    function readProgress() {
        ...
        const elapsedText = document.querySelector(".playbackTimeline__timePassed")?.textContent ||
            document.querySelector('[class*="playbackTimeline__timePassed"]')?.textContent ||
            null;
        const durationText = document.querySelector(".playbackTimeline__duration")?.textContent ||
        ...
        const progressEl = document.querySelector('[role="progressbar"].playbackTimeline__progressBar') ||
        ...
        if (progressEl) {
            ...
            else if (progressEl.style?.width?.includes("%")) {
                const pct = parseFloat(progressEl.style.width.replace("%", ""));
                ratio = pct / 100;
            }
        }
        return ratio;
    ...
    function isTimelineEventTarget(target) {
        if (!(target instanceof Element))
            return false;
        return !!target.closest(".playbackTimeline, [class*='playbackTimeline'], [role='progressbar']");
    }
    function scheduleManualSeekEndCheck(source) {
        ...
        const tick = () => {
            ...
            const remainingSec = audioTime !== null &&
                audioDuration !== null &&
                audioDuration > 0
                ? audioDuration - audioTime
                : null;
            const isHardEnd = (progress !== null && progress >= 0.9985) ||
    ...
    function evaluateEnd(source) {
        ...
        if (trackChanged) {
            ...
            const expectedAdvanceReached = !!state.pendingExpectedUrl &&
                ((currentTrackHref && urlsRoughlyMatch(currentTrackHref, state.pendingExpectedUrl)) ||
                    (urlsRoughlyMatch(currentUrl, state.pendingExpectedUrl) &&
                        isPlaying(getModernTrackPageButton(state.pendingExpectedTitle)) === true));
            const unexpectedTrackWhilePending = !!state.pendingExpectedUrl &&
        ...
        const endedByAudioTime = audioTime !== null &&
            audioDuration !== null &&
            audioDuration > 0 &&
            audioTime > 1 &&
        ...
        if (nearEnd && state.nearEndSince === null)
            state.nearEndSince = Date.now();
        if (!nearEnd)
            state.nearEndSince = null;
        const endedByAria = playing === false && nearEnd;
        const endedByStall = state.nearEndSince !== null &&
            Date.now() - state.nearEndSince > 4000 &&
            playing !== true;
        const endedByProgressReset = progress !== null &&
            prevProgress >= 0.9 &&
            progress <= 0.05 &&
            playing !== true;
        const endedByNoProgressUpdates = playing === false &&
            prevProgress >= 0.9 &&
            prevProgressAt > 0 &&
        ...
        const remainingSec = audioTime !== null &&
            ...
            ? (audioDuration - audioTime)
            : null;
        const endedPreemptivelyByNearEnd = !state.endedReported &&
            playing === true &&
        ...
        if ((audioEnded || endedByAudioTime || endedByAria || endedByStall || endedByProgressReset || endedByNoProgressUpdates) &&
            !state.endedReported) {
    ...
    function bindAudioEndListener() {
        if (!shuffleRuntime.isShuffling)
            return;
        const audio = getAudioEl();
        if (!audio || audio === state.boundAudioEl)
            return;
        unbindAudioEndListener();
    ...
    function bindTimelineEndSeekWatcher() {
        if (timelineEndSeekWatcherBound)
            return;
        timelineEndSeekWatcherBound = true;
        const onSeekInteraction = (event) => {
            if (!shuffleRuntime.isShuffling)
                return;
            if (!isTimelineEventTarget(event.target))
                return;
    ...
    function ensureFooterButtons() {
        ...
        if (!controlsHost || document.getElementById("sc-shuffle-footer-group"))
            return;
        const group = document.createElement("div");
        group.id = "sc-shuffle-footer-group";
        const prevBtn = createIconButton("sc-shuffle-prev", '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M4 3h1v10H4V3zm8 0v10L6 8l6-5z"/></svg>', "Shuffle Prev", () => sendToBackground({ type: "SKIP_PREV" }));
        const stopBtn = createIconButton("sc-shuffle-stop", '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M4 4h8v8H4z"/></svg>', "Stop Shuffle", () => {
        ...
        const nextBtn = createIconButton("sc-shuffle-next", '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M11 3h1v10h-1V3zM4 3v10l6-5-6-5z"/></svg>', "Shuffle Next", () => sendToBackground({ type: "SKIP_NEXT" }));
    ...
    function getHeroActionsContainer() {
        return (document.querySelector(".fullHero__actions") ||
    ...
    function getShareButton() {
        const btn = document.querySelector("button.sc-button-share") ||
    ...
    function ensureHeroButtons() {
        ...
        const shouldShowAny = isLikes || isProfileRoot || isReposts || isTracksTab || isPlaylistsTab || isPlaylist;
        ...
        const container = (shareBtn?.parentElement ? shareBtn.parentElement : null) ||
